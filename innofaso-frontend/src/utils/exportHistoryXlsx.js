...
export async function exportHistoryToXlsx(zonesData) {
  ...
  const groups = {};
  for (const env of ENVIRONMENTS) {
    // pointType peut être un nombre (1,2,3,4), une string ("1"…"4"), ou absent.
    // On normalise en string pour la comparaison.
    groups[env] = {
      fixed:  allFixed.filter((s) => String(s.pointType ?? s.pointId?.split(".")[0] ?? "") === env),
      random: allRandom.filter((s) => String(s.pointType ?? s.pointId?.split(".")[0] ?? "") === env),
    };
  }

  // Fallback : si tous les groupes sont vides (pointType absent de l'API),
  // on regroupe par premier segment de l'identifiant (E.S.N → E = Environnement).
  const allGroupsEmpty = ENVIRONMENTS.every(env => groups[env].fixed.length === 0 && groups[env].random.length === 0);
  if (allGroupsEmpty) {
    for (const env of ENVIRONMENTS) {
      groups[env] = {
        fixed:  allFixed.filter((s) => String(s.pointId).startsWith(env + ".")),
        random: allRandom.filter((s) => String(s.pointId).startsWith(env + ".")),
      };
    }
  }
